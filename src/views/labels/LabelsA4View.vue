...
const mode = ref<'barcode-only' | 'full'>('full')
const cols = ref(3)
  ...
  try {
    ...
    for (let idx = 0; idx < cells.length; idx++) {
      ...
      if (mode.value === 'barcode-only') {
        // Apenas código de barras + número
        try {
          ...
          const bcW = cellW - 2
          doc.addImage(img, 'PNG', x + 1, y + (cellH_mm - bcH) / 2, bcW, bcH)
        } catch { /* ignora */ }
        ...
        if (cell.barcode) {
          try {
            ...
            const bcW = cellW - 4
            doc.addImage(img, 'PNG', x + 2, curY, bcW, bcH)
            curY += bcH + 1
          } catch { /* ignora */ }
        }
