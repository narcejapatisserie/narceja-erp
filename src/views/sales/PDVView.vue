<template>
  <div class="flex flex-col lg:flex-row gap-4 h-full min-h-0">
    ...
    <div class="flex-1 space-y-4 min-w-0">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3 max-h-[calc(100vh-280px)] overflow-y-auto">
        <button
    ...
    <!-- Carrinho -->
    <div class="lg:w-96 flex flex-col card overflow-hidden">
      <div class="p-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
        <h3 class="font-semibold text-gray-900 dark:text-white">
          Carrinho <span v-if="cart.totalItems > 0" class="badge-info badge ml-1">{{ cart.totalItems }}</span>
        </h3>
        <button v-if="cart.items.length > 0" @click="cart.clearCart()" class="text-xs text-red-500 hover:text-red-700">
          Limpar
        </button>
      </div>
      ...
      <div class="flex-1 overflow-y-auto p-3 space-y-2">
        ...
        <div
          ...
          :key="item.product_id"
          class="flex items-center gap-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg p-2.5"
        >
          ...
          <p class="w-20 text-right text-sm font-semibold text-narceja-700 dark:text-narceja-400 flex-shrink-0">
      ...
      <div class="border-t border-gray-100 dark:border-gray-700 p-4 space-y-3">
        ...
        <div v-if="cart.paymentMethod === 'cash'" class="flex items-center gap-2">
          ...
          <span v-if="cart.changeAmount > 0" class="text-xs text-green-600 font-medium whitespace-nowrap">
            Troco: {{ formatCurrency(cart.changeAmount) }}
          </span>
        </div>
